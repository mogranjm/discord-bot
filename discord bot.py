...
async def on_member_join(member):
    ...
    if sys_channel:
        try:
            await sys_channel.send('Hey '+member.mention+' welcome to '+member.guild.name+'! To see all my functions, type "bot functions"!')
...
async def on_message(message):
    ...
    america=['NA','na','america','America','north america','North america','north America', 'North America']
    ...
    if len(message.content)==6:
        if message.content==message.content.upper():
            ...
            if bet == True:
                ...
                channel=client.get_channel(833891322029015060)#CODE_CHANNEL)
                ...
                if sent!=channel:
                    ...
                    if guild==GUILD:
                        ...
                        region=file.read()
                        file.close()
                        await channel.send(f'suspected among us code: {message.content}')
    ...
    if "goodnight" in message.content:
        ...
        await message.channel.send(f'Hey {author} dont forget to record your sleeping times!')
    ...
    if "bot functions" in message.content and on=="True":
        ...
        if on=="True":
            if message.author.id==484893096761622554:
                ...
                await message.channel.send("you dont have permission to turn me off >:(")
        else:
        ...
        if on=="False":
            if message.author.id==484893096761622554:
                ...
                await message.channel.send("you dont have permission to turn me on >:(")
        else:
        ...
        message.author=str(message.author)
        author=message.author.split("#")[0]
        await user.send(f'"{author}" {message.content} in {message.guild.name}!')
        
    elif "484893096761622554" in message.content:
        message.author=str(message.author)
        author=message.author.split("#")[0]
        message.content=str(message.content)
        message.content=message.content.replace("<@!484893096761622554>","@thedepresseddonkey")
    ...
    if not guild:
        if message.author.name==USER:
            if "special functions" in message.content:
                ...
                try:
                    agree= webdriver.find_element_by_xpath('//*[@id="yDmH0d"]/c-wiz/div/div/div/div[2]/div[1]/div[4]/form/div[1]/div/button')
                    agree.click()
                    sleep(3)
                except Exception as error:
                    print(error)
                ...
                elems = webdriver.find_elements_by_xpath('//*[@id="thumbnail"]')
                torf=True
                for elem in elems:
                    if torf==True:
                ...
                if message.author.id==484893096761622554:
                    message.content=message.content.replace("bot send message to ","")
                    count=0
                    count2=0
                    loop=bool(True)
                    while loop == True:
                    ...
                    try:
                        cookies=webdriver.find_element_by_xpath('/html/body/div[2]/div/div/button[1]')
                        cookies.click()
                        sleep(3)
                    ...
                    try:
                        no=webdriver.find_element_by_xpath('/html/body/div[3]/div/div/div/div[3]/button[2]')
                        no.click()
                        sleep(5)
                    except:
                        pass
                    ...
                    choose= webdriver.find_element_by_xpath('//*[@id="react-root"]/section/div/div[2]/div/div/div[1]/div[1]/div/div[3]/button')
                    choose.click()
                    sleep(3)

                    writeName=webdriver.find_element_by_xpath('/html/body/div[5]/div/div/div[2]/div[1]/div/div[2]/input')
                    writeName.send_keys(name)
                    sleep(5)

                    who=webdriver.find_element_by_xpath('/html/body/div[5]/div/div/div[2]/div[2]/div[1]/div/div[3]/button')
                    who.click()
                    sleep(3)

                    Next=webdriver.find_element_by_xpath('/html/body/div[5]/div/div/div[1]/div/div[2]/div/button')
                    Next.click()
                    sleep(3)

                    writeMessage= webdriver.find_element_by_xpath('//*[@id="react-root"]/section/div/div[2]/div/div/div[2]/div[2]/div/div[2]/div/div/div[2]/textarea')
                    writeMessage.send_keys(text)

                    send=webdriver.find_element_by_xpath('//*[@id="react-root"]/section/div/div[2]/div/div/div[2]/div[2]/div/div[2]/div/div/div[3]/button')
                    send.click()

                    await message.channel.send('message "'+text+'" sent successfully to "'+name+'"!')


            
            else:
                await message.channel.send("This was not a command, try typing 'bot functions' for my normal functions or type 'special functions' for more personal features!")
